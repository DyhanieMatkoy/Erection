...
from src.views.person_form import PersonForm

class PersonListFormV2(GenericListForm):
    """
    New Person List Form using Generic List Architecture.
    Supports hierarchy (groups).
    """
    def __init__(self, user_id: int = 4):
        super().__init__("persons", user_id, Person)
        ...
        self.configure_columns([
            {'id': 'full_name', 'name': 'ФИО', 'width': 300},
            {'id': 'position', 'name': 'Должность', 'width': 200},
    ...
    def handle_open_request(self, object_id: int):
        ...
        if item_data:
            ...
            if is_group:
                self.enter_group(object_id, getattr(item_data, 'full_name', ''))
            else:
                self.open_person_form(object_id)
    ...
    def open_person_form(self, person_id: int):
        try:
            form = PersonForm(person_id, parent_id=self.current_parent_id)
    ...
    def on_search(self, text: str):
        ...
        if text:
            ...
            self.controller.set_filter('full_name', text) # Search by full_name
            self.filter_bar.set_navigation_state(False, f"Поиск: {text}")
        else:
            self.controller.set_filter('full_name', None)
    ...
    def on_command_create_group(self):
        """Create new group"""
        try:
            form = PersonForm(0, is_group=True, parent_id=self.current_parent_id)
            form.setAttribute(Qt.WidgetAttribute.WA_DeleteOnClose)
            self.opened_forms.append(form)
            form.destroyed.connect(lambda: self.opened_forms.remove(form) if form in self.opened_forms else None)
            form.destroyed.connect(lambda: self.load_data())
