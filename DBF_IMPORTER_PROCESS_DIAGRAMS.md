...
### 3.1 Unit ID Mapping Data Structure
```mermaid
graph LR
    ...
    subgraph "Transformed Unit Data"
        TU1[DB_ID: 1954091269]
        TU2[DB_ID: 1752670130]
        TU3[DB_ID: 2814749767]
    end
    
    subgraph "Unit ID Mapping Dictionary"
        M1["12345": 1954091269]
        M2["67890": 1752670130]
        M3["ABCDEF": 2814749767]
    end
...
```mermaid
flowchart TD
    A[Start: Read SC12.DBF] --> B[Transform: Convert DBF IDs]
    B --> C[Map: Apply Unit ID Mapping]
    C --> D[Validate: Check unit_id exists in mapping]
    D --> E[Update: Set correct unit_id or None]
    E --> F[Import: Batch insert to works table]
    
    G[Parallel: Unit Mapping Created] --> C
    G -.-> H[Mapping Dictionary: {dbf_id: db_id}]
    H --> E
```
...
DBF_FIELD_MAPPING = {
    ...
    "nomenclature": {
        ...
        "fields": {
            ...
            "PARENTID": "parent_id",
            "CODE": "code",
            "SP17": "unit_id",  # Critical: Links to units table
            "SP15": "price",
            "SP31": "labor_rate",
            "ISMARK": "marked_for_deletion"
        }
...
- **Root Cause**: Unit ID mapping was created during individual entity imports, causing loss of mapping between different import calls
- **Impact**: Works would have `unit_id = None` instead of valid unit references

### 5.2 Solution Architecture
```mermaid
graph TD
    A[Problem: Lost Unit References] --> B[Solution: Centralized Unit Mapping]
    
    subgraph "Before Fix"
        C1[Import Units] --> C2[Create Mapping in import_entity]
        C3[Import Works] --> C4[Create New Mapping in import_entity]
        C5[Result: Mapping Lost Between Calls]
    end
    
    subgraph "After Fix"
        D1[Import All] --> D2[Create Unit Mapping First]
        D3[Import Units] --> D4[Reuse Existing Mapping]
        D5[Import Works] --> D6[Use Existing Mapping]
        D7[Result: Correct Unit References]
    end
```

### 5.3 Implementation Strategy
1. **Preprocessing Phase**: Create unit mapping before any entity imports
2. **Mapping Persistence**: Store mapping in instance variable throughout import session
3. **Reference Resolution**: Apply mapping during works import
