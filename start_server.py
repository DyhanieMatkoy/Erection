...
from api.endpoints import auth, references, documents, registers, costs_materials
...
if web_dist_path.exists():
    # Mount static files for /ctm/ path
    app.mount("/ctm/assets", StaticFiles(directory=str(web_dist_path / "assets")), name="assets")
    ...
    # Serve SPA for /ctm/ paths
    @app.get("/ctm")
    @app.get("/ctm/{full_path:path}")
    async def serve_ctm(full_path: str = ""):
        """Serve the SPA for /ctm/ paths"""
        if full_path == "":
            return FileResponse(web_dist_path / "index.html")
