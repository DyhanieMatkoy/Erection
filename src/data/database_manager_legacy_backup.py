...
# Import SQL translator for multi-database support
try:
    # Add project root to path for SQL translator import
    ...
    logger.warning(f"SQL translator not available: {e}")
...
class DatabaseManager:
    ...
    _sql_translator: Optional['SQLDialectTranslator'] = None  # SQL translator for multi-database support
    ...
    def initialize(self, config_path: str = "env.ini") -> bool:
        ...
        try:
            ...
            try:
                ...
                # Initialize SQL translator for multi-database support
                if SQL_TRANSLATOR_AVAILABLE:
                    self._sql_translator = SQLDialectTranslator(logger)
                    logger.debug("SQL translator initialized for multi-database support")
                else:
                    self._sql_translator = None
                    logger.warning("SQL translator not available - multi-database translation disabled")
    ...
    def _initialize_legacy(self, db_path: str) -> bool:
        ...
        try:
            ...
            # Initialize SQL translator for multi-database support (legacy mode)
            if SQL_TRANSLATOR_AVAILABLE:
                self._sql_translator = SQLDialectTranslator(logger)
                logger.debug("SQL translator initialized for legacy mode")
            else:
                self._sql_translator = None
                logger.warning("SQL translator not available in legacy mode")
    ...
    def _get_current_sql_dialect(self) -> 'SQLDialect':
        """Get current SQL dialect based on database configuration
        
        Returns:
            SQLDialect enum value for current database type
        """
        if not SQL_TRANSLATOR_AVAILABLE:
            return None
    ...
    def _create_tables(self):
        """Create all database tables with automatic SQL translation for multi-database support"""
        cursor = self._connection.cursor()
        ...
        tables = [
            ...
            # These tables MUST match Universal Database Manager schema exactly
        ...
        # Create tables with SQL translation for multi-database support
        for table_sql in tables:
            try:
                # Translate SQL if translator is available and target dialect is SQLite
                if self._sql_translator and target_dialect == SQLDialect.SQLITE:
                    # The original SQL contains DATE types that need to be translated to TEXT for SQLite
                    translated_sql = self._sql_translator.translate_sql(
                        table_sql, 
                        'any',  # Use universal rules for any source to SQLite
                        target_dialect  # Target dialect (SQLite)
                    )
                    logger.debug(f"Translated SQL for {target_dialect.value}: {table_sql[:50]}...")
                else:
                ...
                logger.error(f"Failed to create table with SQL: {table_sql[:100]}...")
    ...
    def _create_indices(self):
        ...
        try:
            ...
            other_indices = [
                ...
                # SYNC TABLE INDICES - CRITICAL for synchronization performance
                ("sync_changes", "CREATE INDEX IF NOT EXISTS idx_sync_changes_node_id ON sync_changes(node_id)"),
                ("sync_changes", "CREATE INDEX IF NOT EXISTS idx_sync_changes_packet_no ON sync_changes(packet_no)"),
                ("sync_changes", "CREATE INDEX IF NOT EXISTS idx_sync_changes_created_at ON sync_changes(created_at)"),
                ("sync_changes", "CREATE INDEX IF NOT EXISTS idx_sync_changes_entity ON sync_changes(entity_type, entity_uuid)"),
                ("sync_changes", "CREATE INDEX IF NOT EXISTS idx_sync_changes_node_operation ON sync_changes(node_id, operation)"),
                ("object_version_history", "CREATE INDEX IF NOT EXISTS idx_object_version_entity ON object_version_history(entity_type, entity_uuid)"),
                ("object_version_history", "CREATE INDEX IF NOT EXISTS idx_object_version_conflict ON object_version_history(entity_type, entity_uuid, resolved_at)")
            ]
