...
class OrganizationListForm(BaseListForm):
    def __init__(self):
    ...
    def setup_ui(self):
        ...
        self.table_view.setHorizontalHeaderLabels(["ID", "Наименование", "ИНН", "Ответственный по умолчанию"])
    ...
    def on_insert_pressed(self):
        """Handle insert key - create new organization"""
    ...
    def on_delete_pressed(self):
        ...
        if current_row >= 0:
            ...
            if id_item:
                ...
                # Check if can delete
                can_delete, usages = self.ref_repo.can_delete_organization(organization_id)
                
                if not can_delete:
                    usage_text = "\n".join([f"- {doc_type}: {doc_info}" for doc_type, doc_info in usages])
                    QMessageBox.warning(
                        self, "Невозможно удалить",
                        f"Организация '{name_item.text()}' используется в следующих документах:\n\n{usage_text}\n\nУдаление невозможно."
                    )
                    return
                
                reply = QMessageBox.question(
                    self, "Подтверждение",
                    f"Пометить на удаление организацию '{name_item.text()}'?",
    ...
    def on_create_group(self):
        """Create new group"""
        form = OrganizationForm(0, is_group=True)
        form.show()
