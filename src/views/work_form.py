...
class WorkForm(QWidget):
    ...
    def load_data(self):
        ...
        cursor.execute("""
            SELECT name, code, unit, price, labor_rate, parent_id, is_group
            FROM works
            WHERE id = ?
        """, (self.work_id,))
        ...
        if row:
            ...
            self.price_spinbox.setValue(row['price'] or 0.0)
            self.labor_rate_spinbox.setValue(row['labor_rate'] or 0.0)
    ...
    def save_data(self):
        ...
        try:
            if self.work_id > 0:
                cursor.execute("""
                    UPDATE works
                    SET name = ?, code = ?, unit = ?, price = ?, labor_rate = ?, parent_id = ?, is_group = ?
                    WHERE id = ?
                """, (self.name_edit.text(), self.code_edit.text(), self.unit_edit.text(),
                      self.price_spinbox.value(), self.labor_rate_spinbox.value(),
                      self.parent_id, 1 if self.is_group else 0, self.work_id))
            else:
                cursor.execute("""
                    INSERT INTO works (name, code, unit, price, labor_rate, parent_id, is_group)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (self.name_edit.text(), self.code_edit.text(), self.unit_edit.text(),
                      self.price_spinbox.value(), self.labor_rate_spinbox.value(),
                      self.parent_id, 1 if self.is_group else 0))
                self.work_id = cursor.lastrowid
