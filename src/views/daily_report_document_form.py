...
class DailyReportDocumentForm(BaseDocumentForm):
    ...
    def create_new_report(self):
        """Create new report"""
        self.date_edit.setDate(QDate.currentDate())
        self.load_unfinished_estimates()
    ...
    def load_report(self):
        ...
        # Load estimates and select current
        self.load_unfinished_estimates()
        for i in range(self.estimate_combo.count()):
            if self.estimate_combo.itemData(i) == row['estimate_id']:
                self.estimate_combo.setCurrentIndex(i)
                break
    ...
    def load_unfinished_estimates(self):
        """Load unfinished estimates"""
        self.estimate_combo.clear()
        estimates = self.service.get_unfinished_estimates()
        
        for estimate in estimates:
            display_text = f"{estimate['number']} от {estimate['date']} - {estimate['customer_name'] or ''}"
            self.estimate_combo.addItem(display_text, estimate['id'])
    
    def on_estimate_changed(self, index):
        """Handle estimate selection change"""
        if index < 0:
            return
        
        estimate_id = self.estimate_combo.itemData(index)
        if not estimate_id:
            return
    ...
    def on_fill_from_estimate(self):
        ...
        estimate_id = self.estimate_combo.currentData()
        if not estimate_id:
    ...
    def recalculate_deviations(self):
        ...
        for row in range(self.table_part.rowCount()):
            try:
                ...
                planned = float(planned_item.text()) if planned_item and planned_item.text() else 0.0
                actual = float(actual_item.text()) if actual_item and actual_item.text() else 0.0
    ...
    def on_save(self):
        ...
        for row in range(self.table_part.rowCount()):
            try:
                ...
                line.planned_labor = float(planned_item.text()) if planned_item and planned_item.text() else 0.0
                line.actual_labor = float(actual_item.text()) if actual_item and actual_item.text() else 0.0
                line.deviation_percent = float(deviation_item.text()) if deviation_item and deviation_item.text() else 0.0
    ...
    def on_print(self):
        ...
        try:
            from ..services.daily_report_print_form import DailyReportPrintForm
            import tempfile
            import os
            
            # Generate PDF
            generator = DailyReportPrintForm()
            pdf_content = generator.generate(self.report_id)
    ...
    def update_posting_state(self):
        ...
        self.estimate_combo.setEnabled(is_editable)
