...
class WorkListForm(BaseListForm):
    ...
    def load_data(self, search_text=""):
        ...
        if search_text:
            cursor.execute("""
                SELECT id, code, name, unit, price, labor_rate, marked_for_deletion
                FROM works
        ...
        for row_idx, row in enumerate(rows):
            ...
            self.table_view.setItem(row_idx, 1, QTableWidgetItem(row['code'] or ""))
            ...
            self.table_view.setItem(row_idx, 3, QTableWidgetItem(row['unit'] or ""))
            self.table_view.setItem(row_idx, 4, QTableWidgetItem(f"{row['price']:.2f}" if row['price'] is not None else ""))
            self.table_view.setItem(row_idx, 5, QTableWidgetItem(f"{row['labor_rate']:.2f}" if row['labor_rate'] is not None else ""))
