...
async def create_work(
    ...
    cursor = db.cursor()
    cursor.execute(
        "INSERT INTO works (name, unit, price, parent_id) VALUES (?, ?, ?, ?)",
        (data.name, data.unit, data.price or 0.0, data.parent_id)
    )
    db.commit()
...
async def update_work(
    ...
    cursor = db.cursor()
    cursor.execute(
        "UPDATE works SET name = ?, unit = ?, price = ?, parent_id = ? WHERE id = ?",
        (data.name, data.unit, data.price or 0.0, data.parent_id, item_id)
    )
    db.commit()
